...
// Componente che controlla l'autenticazione all'avvio
const AuthCheck = () => {
  const { login } = useAuth();
  
  useEffect(() => {
    console.log('Controllo autenticazione all\'avvio');
    const authData = loadAuth();
    
    if (authData) {
      console.log('Dati di autenticazione trovati');
      login(authData.user, authData.token);
      toast.success('Bentornato, ' + authData.user.name);
    }
  }, [login]);
